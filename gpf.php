...
{
	//echo '<h4 align="center" style="border: 2px solid #000000;">Schedule of General Providend Fund Deduction of other than Class-IV (Page:'.($n+1).')</h4>';
	echo '<h4 align="center" style="border: 1px solid #000000;">Schedule of General Providend Fund Deduction</h4>';
	echo '<h4 align="center" >other than Class-IV Page:'.($n+1).'</h4>';
			
	echo '<h4 align="center">'.$GLOBALS['college'].'</h3>';
	echo '<h4 align="center">Under Head: 8009 State Provident Fund</h4>';
	echo '<h4 align="center">Amount deducted from salary for the month of '.$remark.' (Bill: '.$_POST['bill_group'].'-'.$_POST['bill_number'].')</h4>';
	...
	if($n==($GLOBALS['total_pages']-1))
	{
		echo '<table><tr><td align="right">Total in Words: '.Numbers_Words::toWords($a3['GPF_non_IV_9670(-)'],"en_US").' Only</td></tr>
				</table>';
	}
...
{
	 $pay=$d['Pay_of_Officer_0101(+)']+ $d['Grade_Pay_of_Officer_0101(+)']+
			$d['Pay_of_Establishment_0102(+)']+ $d['Grade_Pay_of_Establishment_0102(+)']
			+ $d['NPA_0128(+)'];
	
			echo '<tr>';
		echo '<td>'.($n+1).'</td>';
		echo '<td align="left">'.$d['gpf_acc'].'</td>';
